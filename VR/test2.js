...
class StateMapDrawer {
    ...
    async drawMap() {
        ...
        this.initializeMap();
        await this.fetchShapefileComponents('departamentos', 'municipios');
        await this.drawShapefile('departamentos', 'municipios');
     ...
     */
    async fetchShapefileComponents(mapObject1, mapObject2) {
        const shp = `${this.static_path}/${this.provinceSlug}/${mapObject1}.shp`;
        const dbf = `${this.static_path}/${this.provinceSlug}/${mapObject1}.dbf`;
        const shx = `${this.static_path}/${this.provinceSlug}/${mapObject1}.shx`;
        const shp2 = `${this.static_path}/${this.provinceSlug}/${mapObject2}.shp`;
        const dbf2 = `${this.static_path}/${this.provinceSlug}/${mapObject2}.dbf`;
        const shx2 = `${this.static_path}/${this.provinceSlug}/${mapObject2}.shx`;

        const headers = {'Content-Type': 'application/shapefile+zip; charset=latin1'};

        const [shpBuffer, dbfBuffer, shxBuffer, shpBuffer2, dbfBuffer2, shxBuffer2] = await Promise.all([
            ...
            fetch(shp2, {headers}).then(response => response.arrayBuffer()),
            fetch(dbf2, {headers}).then(response => response.arrayBuffer()),
            fetch(shx2, {headers}).then(response => response.arrayBuffer())
        ]);

        return {shpBuffer, dbfBuffer, shxBuffer, shpBuffer2, dbfBuffer2, shxBuffer2};
    }
     ...
     * @returns {Promise<*>}
     */
    async drawShapefile(mapObject1, mapObject2) {
        const {shpBuffer, dbfBuffer, shxBuffer, shpBuffer2, dbfBuffer2, shxBuffer2} = await this.fetchShapefileComponents(mapObject1, mapObject2);

        return shapefile
            .open(shpBuffer, dbfBuffer, shxBuffer, shpBuffer2, dbfBuffer2, shxBuffer2)
            .then(source => {
