...
class Ventana:
    def __init__(self, window):
        ...
        ################################################################
        # Agrego ruta al servidor.
        ################################################################
        self.the_path = Path(__file__).resolve().parent
        self.server_route = os.path.join(self.the_path, 'server', 'server.py')
        ...
        ################################################################
        # Botones para apagar y prender el servidor.
        ################################################################

        self.server_power_on = ttk.Button(self.input_frame, text="Prender", command=lambda: self.try_connection(), width=10)
        self.server_power_on.grid(row=5, column=0, padx=5, pady=5)

        self.server_power_off = ttk.Button(self.input_frame, text="Apagar", command=lambda: self.stop_server(), width=10)
        self.server_power_off.grid(row=5, column=1, padx=5, pady=5)
    ...
    def try_connection(self, ):
        if the_process != "":
            the_process.kill()
            threading.Thread(target=self.server_init, args=(True, 'ks'), daemon=True).start()         
        else:
            threading.Thread(target=self.server_init, args=(True, 's'), daemon=True).start()
    ...
    def server_init(self, *args):
        ...
        if var == True:
            ...
            if action == 'ks':
                print("Killing and starting server...")
            else:
                print("Starting server...")

            the_process = subprocess.Popen([sys.executable, the_route])
